...
/* ======================================================
   Supabase override after legacy/feed lookup
   Ensures product.html uses same DB data as brand-page grid
====================================================== */
try {
  const SUPABASE_URL =
  ...
  if (SUPABASE_URL && SUPABASE_KEY && window.supabase) {
    ...
    const { data: dbProduct, error } = await supabaseClient
      .from("products")
      .select("*")
      .eq("active", true)
      .or(`external_id.eq.${productId},original_id.eq.${productId}`)
      .limit(1)
      .maybeSingle();
    ...
    if (!error && dbProduct) {
      product = {
        ...product,
        ...dbProduct,
        id: dbProduct.external_id || dbProduct.original_id || dbProduct.id,
        product_id: dbProduct.external_id || dbProduct.original_id || dbProduct.id,
        title: dbProduct.title || "",
        product_name: dbProduct.title || "",
        brand: dbProduct.brand_name || dbProduct.brand_slug || "",
        ...
        product_url: dbProduct.affiliate_url || dbProduct.product_url || "",
        affiliate_url: dbProduct.affiliate_url || dbProduct.product_url || "",
      ...
      const relatedQuery = supabaseClient
        .from("products")
        .select("*")
        .eq("active", true)
        .eq("brand_slug", dbProduct.brand_slug)
        .limit(40);

      const { data: dbRelated } = await relatedQuery;

      if (Array.isArray(dbRelated) && dbRelated.length) {
        products = dbRelated.map((p) => ({
          ...p,
  ...
  console.warn("Supabase product override failed:", err);
}
